...
fn play_combat(mut players: Vec<Deck>) -> (usize, Deck) {
    while ! players.iter().any(|deck| deck.is_empty()) {
        let trick: Vec<_> = players.iter_mut().map(|deck| deck.pop_front().unwrap()).collect();
        let (winner_index, &max_num) = trick.iter().enumerate().max_by_key(|(_, &val)| val).unwrap();
        players[winner_index].push_back(max_num);
        players[winner_index].push_back(trick[1 - winner_index]);
    }
    players.into_iter().enumerate().find(|(_, deck)| !deck.is_empty()).unwrap()
}
...
fn play_recursive_combat(mut players: Vec<Deck>) -> (usize, Deck) {
    ...
    loop {
        ...
        let winner_index;
        // recursive game?
        if players.iter().zip(trick.iter()).all(|(deck, &num)| deck.len() >= num) {
            // TODO
            let recurse_players = players.iter().zip(trick.iter())
                    .map(|(deck, &num)| deck.iter().take(num).copied().collect()).collect();
            winner_index = play_recursive_combat(recurse_players).0;
        } else {
            // normal round: higher card wins:
            winner_index = trick.iter().enumerate().max_by_key(|(_, &val)| val).unwrap().0;
        }
        ...
        if players.iter().any(|deck| deck.is_empty()) {
            return players.into_iter().enumerate().find(|(_, deck)| !deck.is_empty()).unwrap();
        }
    }    
}
...
fn calc_score(deck: Deck) -> usize {
    deck.iter().zip((1..=deck.len()).rev()).fold(0, |acc, (&a, b)| acc + a * b)
}
...
pub fn run() -> () {
    ...
    println!("Part one Winner {} with deck: {:?}", winner + 1, &winning_deck);
    println!("Winner scores: {:?}", calc_score(winning_deck));
    let (recursive_winner, winning_deck) = play_recursive_combat(players);
    println!("Part two Winner {} with deck: {:?}", recursive_winner + 1, &winning_deck);
    println!("Winner scores: {:?}", calc_score(winning_deck));
}
